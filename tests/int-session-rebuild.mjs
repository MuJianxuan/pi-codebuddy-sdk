...
//
// Requires: CodeBuddy CLI logged in.
...
import { createSession, openSession, deleteSession } from "cb-session-io";
...
test("openSession + clear + re-add: CC resolves the replaced content", { timeout: 120_000 }, async () => {
	...
	const s2 = openSession({ sessionId: sid, projectPath: CWD, codebuddyDir: process.env.CODEBUDDY_CONFIG_DIR });
	s2.clear();
...
test("deleteSession + createSession({sessionId}): sessionId preserved across full wipe", { timeout: 120_000 }, async () => {
	...
	deleteSession(sid, CWD, process.env.CODEBUDDY_CONFIG_DIR);
