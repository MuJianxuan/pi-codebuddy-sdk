#!/usr/bin/env bash
# Multi-turn integration tests for pi-codebuddy-sdk provider.
# Verifies tool use and multi-turn context via --mode json output.
# Requires: pi CLI, CodeBuddy (for Agent SDK subprocess), jq.
...
run_json() {
  ...
  printf "%-50s " "$name"
  if timeout "$TIMEOUT" "$@" < /dev/null > "$logfile" 2>"$logfile.err"; then
    if [ ! -s "$logfile" ]; then
...
# "pi-codebuddy-sdk"), turn 2 reads LICENSE (has "MIT License").  If stale
