#!/usr/bin/env bash
# Prompt cache efficiency test for pi-codebuddy-sdk.
...
rm -f "$TMPFILE" "$CODEBUDDY_SDK_DEBUG_PATH"
...
timeout 180 pi --no-session -ne -e "$DIR" \
  ...
  < /dev/null > "$LOGFILE" 2>"$LOGFILE.err" || PI_EXIT=$?
PI_EXIT=${PI_EXIT:-0}
...
done < <(grep "syncResult:" "$CODEBUDDY_SDK_DEBUG_PATH" 2>/dev/null || true)
...
if [ "$FAIL" -eq 0 ]; then
  ...
  echo "  Log: $LOGFILE"
  echo "  Debug: $CODEBUDDY_SDK_DEBUG_PATH"
  exit 1
fi
